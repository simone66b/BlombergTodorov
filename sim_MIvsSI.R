...
for(i in 1: length(result1se)){
  resultSE[i] = (sqrt(result1se[i]^2 + result2se[i]^2))/sqrt(seX1^2 + seX2^2)
}

#add the std error twice (for X1 and X2) - it's the same
result$std.error = rep(resultSE, 2)
...
asd <- ggplot(res, aes(y = names, x = estimate)) +
  geom_errorbar(aes(xmin = `2.5 %`, xmax = `97.5 %`), 
                color = cols, alpha = 0.7) +
  geom_vline(aes(xintercept = 0), color = "red", 
             linewidth = 0.5, alpha = 0.8, linetype = "dashed") +  
  geom_point(color = cols, size = 1.5) +
...
asd1 <- ggplot(res, aes(y = std.error, x = names)) +
  ...
  geom_hline(aes(yintercept = 1), color = "red", 
             linewidth = 0.5, alpha = 0.8, linetype = "dashed") +
  geom_point( color=cols, size=2, alpha=0.6) +
  theme_light() +
  coord_flip() +
  theme(
...
#combined plot
plot_grid(asd, asd1, labels = "AUTO", ncol = 2,  rel_widths = c(4, 1.5), align="hv", hjust = 0, vjust = 1.5, scale = 1)
...
gt_tab = gt_tbl |>  tab_header(
  ...
  subtitle = md("Bias of the estimates, std.error and CIs of different imputation results compared to the original full dataset")) |>
  cols_align(
    align = "center")
#output file name
gtsave(gt_tab, "table3.docx")
...
#   facet_grid(.~ num, scales = "free", space = "free") +
#   labs(y = "Std. error", x = "Missingness pattern")
# 
# 
# plot_grid(asd, asd1, labels = "AUTO", nrow = 2,  rel_heights = c(3, 1), align="hv", hjust = -2, vjust = 1.5)



#Alternative
# 
# bp <- ggplot(result[-c(25,50),], aes(x=names, y=estimate))+  geom_boxplot(fill="gray") + coord_cartesian(ylim = c(-0.025, 0.055)) + geom_hline(yintercept=0, color = "red") #geom_violin() + geom_jitter(height = 0, width = 0.1)
# bps <- ggplot(result[-c(25,50),], aes(x=names, y=std.error))+  geom_boxplot(fill="gray") + coord_cartesian(ylim = c(0, 0.025)) + geom_hline(yintercept=0, color = "red")#geom_violin() + geom_jitter(height = 0, width = 0.1)
# plot_grid(bp, bps, labels = "AUTO", nrow = 2)
# 
# final <- ggplot(res[-1,], aes(x=names, y=estimate)) + geom_point() + geom_errorbar(aes(ymin = `2.5 %`, ymax = `97.5 %`)) + geom_hline(yintercept=0, color = "red")
